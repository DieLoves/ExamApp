...
export default function ExamPage() {
	...
	if (exam.showReview) {
		...
		const reviewFooter = (
			<AppFooter>
				...
				<Button
					variant='outline'
					className='h-10 text-base'
					onClick={exam.handleBackToResults}
				>
					К результатам
				</Button>
				<Button
		...
		return (
			<AppLayout headerTitle='Просмотр ответов' footer={reviewFooter}>
				...
				<ExamProgress
					...
					progressPercentage={
						((exam.reviewQuestion + 1) / exam.questions.length) * 100
					}
				/>
	...
	if (exam.showResults) {
		...
		return (
			<AppLayout headerTitle='Результаты тестирования' footer={resultsFooter}>
				{exam.settings && (
					<ExamResults
						...
						correctAnswers={Math.round(
							(exam.finalScore / 100) * exam.questions.length
						)}
						totalQuestions={exam.questions.length}
	...
	const examFooter = (
		<AppFooter>
			...
			<ConfirmExamFinish
				isFinish={
					Object.keys(exam.selectedAnswers).length === exam.questions.length
				}
	...
	return (
		<AppLayout headerTitle='Тестирование' footer={examFooter}>
			<div className='flex items-center justify-between'>
				...
				<Timer
					initialMinutes={exam.settings.timeTaken}
