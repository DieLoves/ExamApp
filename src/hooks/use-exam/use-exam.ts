...
// Добавьте импорт функции setContentProtection
import { setContentProtection } from "@/lib/content-protection"
...
export function useExam() {
  ...
  const [state, setState] = useState<ExamState>({
    ...
    disciplineName: "", // Добавляем название дисциплины
  })
  ...
  const initExam = useCallback(async () => {
    ...
    try {
      ...
      if (savedSettings.selectedModules && savedSettings.selectedModules.length > 0) {
        filteredQuestions = questionsData.questions.filter((q: IExamFileQuestions) =>
      ...
      const questionCount = Math.min(savedSettings.tasksCount || 30, filteredQuestions.length)
      const originalQuestions = shuffle<IExamFileQuestions>(filteredQuestions).slice(0, questionCount)

      // Перемешиваем ответы и отслеживаем оригинальные индексы
      const questionsWithShuffledAnswers: ShuffledQuestion[] = originalQuestions.map((q) => {
      ...
      updateState({
        ...
        progress: 0,
        isLoading: false,
        disciplineName: discipline.name, // Сохраняем название дисциплины
      })
    } catch (error) {
  ...
  const handleAnswerSelect = useCallback((answerIndex: number) => {
    setState((prevState) => {
      const originalIndex = prevState.questions[prevState.currentQuestion].answers[answerIndex].originalIndex
  ...
  const calculateProgressValue = useCallback(() => {
    return setState((prevState) => {
      ...
      const progressValue = (answeredCount / prevState.questions.length) * 100
      return { ...prevState, progress: isNaN(progressValue) ? 0 : progressValue }
    })
  }, [])
  ...
  const calculateScore = useCallback(() => {
    setState((prevState) => {
      let score = 0
      Object.entries(prevState.selectedOriginalAnswers).forEach(([questionIdx, originalAnswerIdx]) => {
        const question = prevState.questions[Number.parseInt(questionIdx)]
        const originalAnswers = question.answers.filter((a) => a.originalIndex === originalAnswerIdx)
        if (originalAnswers.length > 0 && originalAnswers[0].isCorrect) {
          score++
        }
      })

      const percentage = prevState.questions.length > 0 ? (score / prevState.questions.length) * 100 : 0
  ...
  const handleFinishExam = useCallback(async () => {
    setState((prevState) => {
      ...
      if (prevState.startTime) {
        timeElapsed = Math.floor((now.getTime() - prevState.startTime.getTime()) / 1000)
      }
    ...
    // Отключаем защиту контента при завершении экзамена
    ...
    setState((prevState) => {
      if (prevState.settings) {
        const now = new Date()
        const correctAnswers = Math.round((prevState.finalScore / 100) * prevState.questions.length)
        ...
        saveExamResult({
          ...
          disciplineName: prevState.disciplineName,
        }).catch((error) => {
  ...
  useEffect(() => {
    const saveProgress = async () => {
      ...
      const progress: ExamProgress = {
        ...
        startTime: state.startTime ? state.startTime.toISOString() : new Date().toISOString(),
    ...
    const setupTauriListeners = async () => {
      try {
        ...
        const unlisten = await currentWindow.onCloseRequested(() => {
          handleEarlyTermination("Попытка закрыть приложение во время экзамена")
        })
  ...
  }, [state.isFinished, state.isLoading, state.settings, handleEarlyTermination])
