...
// Обновим интерфейс ExamSettings, чтобы включить выбранную дисциплину
  ...
  try {
    if (isTauri()) {
      ...
      if (store) {
        ...
        const existingResults = ((await store.get("examResults")) as ExamResult[] | null) || []
    ...
    const results = existingResults ? (JSON.parse(existingResults) as ExamResult[]) : []
    results.push(result)
